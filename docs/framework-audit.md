...
- public entrypoints are `createServerWorld()` and `createClientWorld()`
- there is no public `createWorld()`, `World` constructor, game object, or app wrapper
- server/client roles are fixed at construction time
- transports pass raw `Uint8Array` packets plus channel labels; reliability belongs to the server/engine layer
- default visibility is all-visible, with server-owned overrides and interest hooks when needed
...
pnpm test:examples
pnpm bench
rg -n "createWorld|declare class World|BinaryReader|BinaryWriter|ComponentStorage|SparseSetComponentStorage|createSyncServer|createSyncClient" dist/index.d.mts
```
...
- `test/types.test.ts` protects server/client directionality and type-only ergonomics.
...
- Server/client world construction is the only runtime entry model.
- Transport does not rewrap or implement reliability protocol.
- Interest management defaults to visible-all and remains server-defined.
- Prediction/interpolation/rollback are documented as server policy with framework hooks only.
...
- Batched dirty snapshots reduce bytes and can improve client apply, but server send CPU is not a
...
3. Add small server-policy helpers only when examples show repeated friction.
