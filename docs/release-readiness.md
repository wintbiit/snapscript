...
- Package versions are aligned at `0.4.0`.
...
- `0.4.0` removes the public `Clock` service and drives worlds with `world.tick(deltaTime)`.
- `0.4.0` keeps frame timing inside each world: `tick`, `dtMs`, and `nowMs` derive from `deltaTime`.
